...
export const calculateMaxDebtRepayable = (
	...
	const totalDebtValue = getTotalValueOfCoinArray(debts, prices)
	const ltCollateralValue = getTotalLiquidationThresholdValueOfCollateral(
		collaterals,
		prices,
		assetParams,
	)
...
export const getTotalLtvValueOfCollateral = (
...
export const getTotalLiquidationThresholdValueOfCollateral = (
	collateral: Collateral[],
	prices: Map<string, BigNumber>,
	assetParams: Map<string, AssetParamsBaseForAddr>,
) => {
	return collateral.reduce((acc, collateral) => {
		...
		const liquidationThreshold = new BigNumber(
			assetParams.get(collateral.denom)?.liquidation_threshold || 0,
		)
...
export const getLiquidationThresholdHealthFactor = (
	...
	const totalDebtValue = getTotalValueOfCoinArray(debts, prices)
	const totalCollateralValue = getTotalLiquidationThresholdValueOfCollateral(
		collateral,
		prices,
		assetParams,
	)
