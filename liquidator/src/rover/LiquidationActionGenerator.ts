...
export class LiquidationActionGenerator {
  ...
  constructor(osmosisRouter: AMMRouter, markets: MarketInfo[]) {
  ...
  produceBorrowActions = (debt: Debt, collateral: Collateral): Action[] => {
    ...
    const marketInfo: MarketInfo | undefined = this.markets.find(
      (market) => market.denom === debt.denom,
    )
    if (
      !marketInfo ||
  ...
  borrowWithoutLiquidity = (debtCoin: Coin): Action[] => {
    ...
    const bestMarket = this.markets
      .filter((market) => market.borrow_enabled && market.denom !== debtdenom)
   ...
   */
  generateSwapActions = (
    collateralWonDenom: string,
    debtBorrowedDenom: string,
    debtAmount: string,
  ): Action[] => {
