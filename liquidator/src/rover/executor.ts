...
export class Executor extends BaseExecutor {
  ...
  start = async () => {
    ...
    let subAccounts = await this.queryClient.queryContractSmart(this.config.accountNftAddress, {
    ...
    subAccounts = await this.queryClient.queryContractSmart(this.config.accountNftAddress, {
      tokens: { owner: this.config.liquidatorAddress },
    })
  ...
  run = async () => {
    ...
    const swapToStableMsg =
      bestCollateral.denom !== this.config.neutralAssetDenom
    ...
    const msg = {
      update_credit_account: { account_id: this.liquidatorAccountId, actions: actions },
    }
  ...
  findBestCollateral = (collaterals: Coin[], vaultPositions: VaultPosition[]): Collateral => {
    ...
    const largestCollateralVault = vaultPositions
      .sort((vaultA, vaultB) => this.calculateVaultValue(vaultA) - this.calculateVaultValue(vaultB))
      .pop()

    const bestCollateral: Coin | VaultPosition | undefined =
      this.calculateCoinValue(largestCollateralCoin) >
      this.calculateVaultValue(largestCollateralVault!)
  ...
  calculateVaultValue = (vault: VaultPosition | undefined): number => {
    ...
    const vaultAmount = vaultAmountLocked.isGreaterThan(vaultAmountUnlocked)
      ? vaultAmountLocked
      : vaultAmountUnlocked
    ...
    return largestVaultValue.isGreaterThan(largestUnlockingValue)
