...
export const run = async (liquidationHelper: LiquidationHelper, redis: IRedisInterface) => {
  ...
  positionData.forEach(async (positionResponse: DataResponse) => {
    ...
    if (walletBalance > totalNewDebt) {
      ...
      console.log(`- WARNING: Cannot liquidate liquidatable position for ${tx.user_address} because we do not have enough assets.`)
      console.log({unliquidatableDebtSize: amount, scheduledForLiquidation: debtAmount, liquidatorBalance: walletBalance, denom:tx.debt_denom})
    }
  })
  ...
  for (const index in results) {
    ...
    Object.keys(collaterals).forEach((denom: string) => msgs.push(
      makeWithdrawMessage(
        liquidatorAddress,
        denom, 
        addresses.redBank))) 
    ...
    Object.keys(swaps).forEach((route: string)=>msgs.push(makeSwapMessage(
      liquidatorAddress,
      swaps[route],
      ROUTES[route]
    )))
