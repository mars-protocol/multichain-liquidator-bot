...
const RedbankExecutor_js_1 = require("./redbank/RedbankExecutor.js");
...
const main = async () => {
    ...
    const sm = (0, secretManager_js_1.getSecretManager)();
    ...
    const network = networkEnv === "MAINNET" ? network_js_1.Network.MAINNET : networkEnv === "TESTNET" ? network_js_1.Network.TESTNET : network_js_1.Network.LOCALNET;
...
const launchRedbank = async (client, wasmClient, redbankConfig, poolProvider, exchangeInterface, apiRequester) => {
    await new RedbankExecutor_js_1.RedbankExecutor(redbankConfig, client, wasmClient, poolProvider, exchangeInterface, apiRequester).start();
};
